...
int main(void){
    ...
    int16_t Strassen_Winograd_pre_C00[SQUARE_DIM / 2][SQUARE_DIM / 2];
    int16_t Strassen_Winograd_pre_C01[SQUARE_DIM / 2][SQUARE_DIM / 2];
    int16_t Strassen_Winograd_pre_C10[SQUARE_DIM / 2][SQUARE_DIM / 2];
    int16_t Strassen_Winograd_pre_C11[SQUARE_DIM / 2][SQUARE_DIM / 2];
    ...
    for(size_t i = 0; i < SQUARE_DIM / 2; i++){
        for(size_t j = 0; j < SQUARE_DIM / 2; j++){
            ...
            Strassen_Winograd_pre_C00[i][j] = Strassen_Winograd_pre_C01[i][j] = 0;
            Strassen_Winograd_pre_C10[i][j] = Strassen_Winograd_pre_C11[i][j] = 0;
...
// ================================
    ...
    for(size_t i = 0; i < SQUARE_DIM / 2; i++){
        for(size_t j = 0; j < SQUARE_DIM / 2; j++){
            Strassen_Winograd_pre_res[i                   ][j                   ] = Strassen_Winograd_pre_C00[i][j];
            Strassen_Winograd_pre_res[i                   ][j + (SQUARE_DIM / 2)] = Strassen_Winograd_pre_C01[i][j];
            Strassen_Winograd_pre_res[i + (SQUARE_DIM / 2)][j                   ] = Strassen_Winograd_pre_C10[i][j];
            Strassen_Winograd_pre_res[i + (SQUARE_DIM / 2)][j + (SQUARE_DIM / 2)] = Strassen_Winograd_pre_C11[i][j];
        }
    }
    }
    end = rdtsc();
...
// ================================
    ...
    for(size_t i = 0; i < SQUARE_DIM / 2; i++){
        for(size_t j = 0; j < SQUARE_DIM / 2; j++){
            Strassen_A00[i][j] = rand();
            Strassen_A01[i][j] = rand();
            Strassen_A10[i][j] = rand();
            Strassen_A11[i][j] = rand();
            Strassen_B00[i][j] = rand();
            Strassen_B01[i][j] = rand();
            Strassen_B10[i][j] = rand();
            Strassen_B11[i][j] = rand();
        }
    }

    start = rdtsc();
    for(size_t i = 0; i < 16; i++){
        Strassen_Winograd_pre_square_matmul(
            &Strassen_Winograd_pre_C00[0][0], &Strassen_Winograd_pre_C01[0][0],
            &Strassen_Winograd_pre_C10[0][0], &Strassen_Winograd_pre_C11[0][0],
    ...
    ns = (end - start);
    printf("Strassen_Winograd with preprocessing cycles:\n%lld\n", ns);

    for(size_t i = 0; i < SQUARE_DIM / 2; i++){
        for(size_t j = 0; j < SQUARE_DIM / 2; j++){
            Strassen_Winograd_pre_res[i                   ][j                   ]
                = Strassen_Winograd_pre_C00[i][j];
            Strassen_Winograd_pre_res[i                   ][j + (SQUARE_DIM / 2)]
                = Strassen_Winograd_pre_C01[i][j];
            Strassen_Winograd_pre_res[i + (SQUARE_DIM / 2)][j                   ]
                = Strassen_Winograd_pre_C10[i][j];
            Strassen_Winograd_pre_res[i + (SQUARE_DIM / 2)][j + (SQUARE_DIM / 2)]
                = Strassen_Winograd_pre_C11[i][j];
        }
    }
    ...
    for(size_t i = 0; i < SQUARE_DIM; i++){
        for(size_t j = 0; j < SQUARE_DIM; j++){
            if(A[i][j] != Strassen_Winograd_pre_res[i][j]){
                fprintf(stderr, "%4zu, %4zu: %8d, %8d\n", i, j,
                    A[i][j], Strassen_Winograd_pre_res[i][j]);
            }
        }
    }
#endif
...
void Strassen_Winograd_pre_square_matmul(
    ...
    // A10 + A11
    matrix_add_fromM(&T0[0][0], Strassen_A10, Strassen_A11, dim, dim,
        A + dim * 2 * dim, A + dim * 2 * dim + dim, 2 * dim);
    // B01 - B00
    matrix_sub_fromM(&T1[0][0], Strassen_B01, Strassen_B00, dim, dim,
        B + dim, B, 2 * dim);
    ...
    // A00 + A01 - A10 - A11
    matrix_sub_negacc_fromM1(&T0[0][0], Strassen_A01, Strassen_A11, dim, dim,
        A + dim, 2 * dim);
    // B01 + B10 - B00 - B11
    matrix_sub_acc_fromM1(&T1[0][0], Strassen_B10, Strassen_B00, dim, dim,
        B + dim * 2 * dim, 2 * dim);
    // A00 * B00 + A01 * B10
